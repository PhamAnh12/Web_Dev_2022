...
using Misa_WebDev2022_Api.Entities;
...
namespace MISA.WebDev2022.Api.Controllers
{
    ...
    public class EmployeesController : ControllerBase
    {
        ...
        [SwaggerResponse(StatusCodes.Status400BadRequest)]
        [SwaggerResponse(StatusCodes.Status500InternalServerError)]
        public IActionResult FilterEmployees([FromQuery] string? search, [FromQuery] Guid? positionID, [FromQuery] Guid? departmentID, [FromQuery] string? sort, [FromQuery] int pageSize = 10, [FromQuery] int pageNumber = 1)
        {
            try
            {
                // Khởi tạo kết nối tới DB MySQL
                ...
                if (search != null)
                {
                    whereConditions.Add($" ((EmployeeCode LIKE  \'%{search}%\' OR PhoneNumber LIKE  \'%{search}%\' OR EmployeeName LIKE  \'%{search}%\'))");
                }
                if (positionID != null)
                {
